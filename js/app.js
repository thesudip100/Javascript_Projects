...
addNoteButton.addEventListener("click", function (e) {
    ...
    if (notes_check == null) {
        ...
        //if some notes are found, parse those notes from array and store in notesObj. We can iterate notesObj since it returns array.
    ...
    // we are pushing the value of that textarea i.e what we write in the textarea is being added in the notesObj which is an array.
    notesObj.push(add_Text.value);

    //We need to set the added text value in the local storage under the name "notes". Since notesObj is an array and we cannot set array directly in the local storage as we have learned so we use the function JSON.stringify().
    localStorage.setItem("notes", JSON.stringify(notesObj));

    //After we type sth on textarea and click on "Add note", this will give us the blank text area again since we do not want the previously entered value to be stucked there.
    add_Text.value = "";
...
function showNotes() {
    //We had set notes in the local storage. To show them , we need to fetch the note first. If notes are not found, if part will be executed and if found, else part will be executed.
    let notes_check = localStorage.getItem("notes");
    if (notes_check == null) {
        notesObj = [];
    }
    ...
    notesObj.forEach(function (element, index) {
        ...
        <div class="noteCard my-2 mx-2 card" style="width: 18rem;">
            ...
            //Suppose, we have a key value pair in the local storage under the name notes and that notes consists array ["hello","hi","how are you"]. The index of the first note is 0 , second is 1 and third is 2. When we iterate the first element of the array, i.e "hello", then we want it to be stored as Note 1. Thats why we wrote ${index+1}

            //We want the iterated element to be present in the paragraph section iteself so we have ${element}

            //When we click Delete button, we want to delete that particular note but not others. There may be many notes and to delete, we need to grab them by id for deleting individually. so we give them their index number as id and onclick i.e when we click on that section, we call deleteNote() function inside which the argument passed is this.id meaning that the the argument is what the "id" is.
    });
    ...
    //check if the length of notesObj is zero or not, if it contains anynotes in it, its length wont be zero. If it is not zero then add the above html in the container inside index.html with id "notepad"
